		...
		gpointer         user_data)
{
	...
	sprintf(timestring,"t: %02.0lf:%02.0lf:%02.0lf", hours, minutes, timetotal);
	GtkWidget * label = lookup_widget(GTK_WIDGET(button), "timer");
		...
		gpointer         user_data)
{
	...
	sprintf(timestring,"s: %02.0lf:%02.0lf:%02.0lf t: %02.0lf:%02.0lf:%02.0lf",shours, sminutes,sinter, hours, minutes, timetotal);
	GtkWidget * label = lookup_widget(GTK_WIDGET(button), "timer");
	gtk_label_set_text(GTK_LABEL(label),timestring);
}
